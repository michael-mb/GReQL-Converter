    ...
    <div class="card-header" :class="!rule.active ? 'disabled' : ''">
      <h5 class="accordion-faq m-0 position-relative">
        <a class="custom-accordion-title text-reset d-block" @click="toggle">
          <span class="type"> SA </span> <span class="type_name">{{rule.rule_type}} : {{rule.class_aggregate}}
          => {{rule.class_element }}
    ...
    <div class="collapse " :class="isOpen === true ? 'show' : ''">
      <div class="card-body" :class="!rule.active ? 'disabled' : ''">
          <div class="form-group row">
            ...
            <div class="col-md-9">
              <input type="text" class="form-control" v-model="rule.class_aggregate">
            </div>
          </div>
          <div class="form-group row">
            <label class="col-form-label col-md-3">Class Element</label>
            <div class="col-md-9">
              <input type="text" class="form-control" v-model="rule.class_element">
            </div>
          </div>
          ...
          <div class="form-group row">
            <label class="col-form-label col-md-3">Element Multiplicity</label>
            <div class="col-md-9">
              <input type="text" class="form-control" v-model="rule.element_multiplicity">
            </div>
          </div>
