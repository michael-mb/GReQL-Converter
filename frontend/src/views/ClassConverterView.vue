      ...
      <div class="content container-fluid">
                  ...
                  <div class="col-md-8">
                    <select class="form-select" v-model="defaultCode" @change="updateDefaultCode">
                      <option v-for="(item, key) in default_test_code" :value="item"
                              :selected="defaultCode === item">{{ key }}
                      </option>
                    </select>
                  </div>
                </div>
                <code-editor font-size="14px" v-model="code" theme="github" 
            ...
            <div class="card">
              ...
              <div class="card-body">
                <div class="accordion custom-accordion" id="custom-accordion-one">
                  <template v-for="(rule, index) in store.rules">
                    <DefineClass v-if="rule.rule_type === rulesDefinitions.RULE_TYPE.defined_class" :rule="rule"
                                 :index="index"/>
                    <DefineEnum v-if="rule.rule_type === rulesDefinitions.RULE_TYPE.defined_enum" :rule="rule"
                                :index="index"/>
                    <Generalization v-if="rule.rule_type === rulesDefinitions.RULE_TYPE.generalization" :rule="rule"
                                    :index="index"/>
                    <SimpleAssociation v-if="rule.rule_type === rulesDefinitions.RULE_TYPE.simple_association"
                                       :rule="rule" :index="index"/>
                    <Aggregation v-if="rule.rule_type === rulesDefinitions.RULE_TYPE.aggregation" :rule="rule"
                                 :index="index"/>
                    <Composition v-if="rule.rule_type === rulesDefinitions.RULE_TYPE.composition" :rule="rule"
                                 :index="index"/>
                    ...
                    <NominationConsistency v-if="rule.rule_type === rulesDefinitions.RULE_TYPE.nomination_consistency"
                                           :rule="rule" :index="index"/>
                    <Count
                        v-if="rule.rule_type === rulesDefinitions.RULE_TYPE.count_methods || rule.rule_type === rulesDefinitions.RULE_TYPE.count_attributes"
                        :rule="rule" :index="index"/>
                  </template>
                ...
                <div class="dropdown dropdown-action" v-if="store.rules.length > 0 ">
                  <a class="mt-3 action-icon dropdown-toggl btn btn-primary" data-bs-toggle="dropdown"
                     aria-expanded="false">
                    <i class="feather-plus-circle"></i> Add Rule
                  </a>
                  <div class="dropdown-menu dropdown-menu-right">
                    <a class="dropdown-item" @click="addRule(rulesDefinitions.RULE_TYPE.defined_class)"> <i
                        class="feather-file-plus me-2"></i> Class definition</a>
                    <a class="dropdown-item" @click="addRule(rulesDefinitions.RULE_TYPE.defined_enum)"> <i
                        class="feather-file-plus me-2"></i> Enum definition</a>
                    <a class="dropdown-item" @click="addRule(rulesDefinitions.RULE_TYPE.generalization)"> <i
                        class="feather-file-plus me-2"></i> Generalization</a>
                    <a class="dropdown-item" @click="addRule(rulesDefinitions.RULE_TYPE.aggregation)"> <i
                        class="feather-file-plus me-2"></i> Aggregation</a>
                    <a class="dropdown-item" @click="addRule(rulesDefinitions.RULE_TYPE.composition)"> <i
                        class="feather-file-plus me-2"></i> Composition</a>
                    <a class="dropdown-item" @click="addRule(rulesDefinitions.RULE_TYPE.simple_association)"> <i
                    ...
                    <a class="dropdown-item" @click="addRule(rulesDefinitions.RULE_TYPE.count_methods)"> <i
                        class="feather-file-plus me-2"></i> Count Methods</a>
                    <a class="dropdown-item" @click="addRule(rulesDefinitions.RULE_TYPE.count_attributes)"> <i
                        class="feather-file-plus me-2"></i> Count Attributes</a>
                    <a class="dropdown-item" @click="addRule(rulesDefinitions.RULE_TYPE.nomination_consistency)"> <i
                        class="feather-file-plus me-2"></i> Nomination Consistency</a>
                  </div>
        ...
        <div class="col-md-12 col-sm-12 d-flex" v-if="!globalUtils.isStringEmpty(GReQLCode)">
          <div class="card">
            ...
            <div class="card-body" ref="greQL_container">
              <code-editor font-size="14px" v-model="GReQLCode" theme="github" @textarea="focus" 
        ...
        <div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvas" aria-labelledby="offcanvasLabel">
          ...
          <div class="blog-image">
            <img v-if="store.getOffCanvas.image !== ''" class="img-fluid" :src="store.getOffCanvas.image"
                 alt="Post Image">
          </div>
